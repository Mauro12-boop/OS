...
class SwimmingPool:
        ...
        with self.recreation_lock:
            if len(self.recreation_pool) < 20:
                ...
                if from_queue:
                    ...
                    # ✅ LOG: Entered recreation pool from queue
                    client.log_activity(
                    ...
                    # ✅ LOG: Entered recreation pool directly
                    client.log_activity(
                        amenity="Swimming Pool Recreation",
    ...
    def _recreation_swim_session(self, client):
        ...
        # ✅ LOG: Finished recreation swimming
        client.log_activity(
            amenity="Swimming Pool Recreation",
            action="Finish swimming",
            success=True,
        ...
        with self.queue_lock:
            if self.lanes_queue:
                ...
                if success:
                    ...
                    # ✅ LOG: Moved from queue to lane
                    client.log_activity(
                        amenity="Swimming Pool Lane",
                        action="Queue to lane",
                        success=True,
                        info="Moved from queue to swimming lane"
                    )
        ...
        with self.queue_lock:
            if self.recreation_queue:
                ...
                if success:
                    ...
                    # ✅ LOG: Moved from queue to recreation pool
                    client.log_activity(
                        amenity="Swimming Pool Recreation",
                        action="Queue to recreation",
                        success=True,
                        info="Moved from queue to recreation pool"
                    )
...
# REMOVED: go_to_pool method - integrated into controller
